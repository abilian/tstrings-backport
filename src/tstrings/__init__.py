...
if TYPE_CHECKING:
    _STR_TO_CONVERSION: dict[str, Conversion] = {"s": "s", "r": "r", "a": "a"}

    def _str_to_conversion(conv_char: str) -> Conversion:
        try:
            return _STR_TO_CONVERSION[conv_char]
        except KeyError:
            raise SyntaxError(f"t-string: invalid-conversion character {conv_char!r}")
...
def t(template_string: str, /) -> Template:
    ...
    for match in _INTERPOLATION_RE.finditer(template_string):
        ...
        if groups["debug"]:
            ...
            if groups["conversion"]:
                conversion = _str_to_conversion(groups["conversion"][1])
            else:
                conversion = None
